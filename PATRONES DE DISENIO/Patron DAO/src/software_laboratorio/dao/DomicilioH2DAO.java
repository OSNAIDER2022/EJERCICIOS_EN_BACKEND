...
public class DomicilioH2DAO implements IDao<Domicilio> {
    ...
    private static final String SQL_DELETE = "DELETE DOMICILIOS WHERE ID = ?;";
    ...
    public List<Domicilio> buscarTodos() {
        ...
        try {
            LOGGER.info("Se ha iniciado la visualizacion de todas los DOMICILIOS");
            connection = DB.getConnection();
            PreparedStatement psAllSearch = connection.prepareStatement(SQL_ALLSEARCH);
            ResultSet rs = psAllSearch.executeQuery();
            while (rs.next()){
                domicilio = new Domicilio(rs.getInt(1),rs.getNString(2),rs.getString(3),rs.getString(4),rs.getString(5));
            ...
            try {
                LOGGER.warn("ADVERTENCIA: Se ha producido un error en la ejecucion de MOSTRAR TODOS LOS DOMICILIOS. Por tanto, se intentará cerrar la conexion con la DB");
                connection.close();
            } catch (SQLException e2) {
                LOGGER.error("ALERTA: Se ha generado un error que ha impedido el cierre de la conexion con la DB");
                e2.printStackTrace();
        ...
        LOGGER.info("Se ha finalizado la visualizacion de todos los DOMICILIOS exitosamente");
